...
/** Outbox Pattern — same design as Inventory Service */
@Entity
@Getter
@Setter
@Builder
@NoArgsConstructor
@AllArgsConstructor
@Table(name = "outbox_events", schema = "schema_order",
        indexes = {
                @Index(name = "idx_order_outbox_status",   columnList = "status"),
                @Index(name = "idx_order_outbox_event_id", columnList = "event_id", unique = true)
        })
